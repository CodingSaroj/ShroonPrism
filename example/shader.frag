...
void main()
{
    ...
    float ndotl = max(dot(f_Normal, u_LightDir), 0.2);
    vec3 diffuse = u_LightCol * ndotl;

    vec3 viewDir = normalize(-f_FragPos);
    vec3 reflectDir = reflect(-u_LightDir, f_Normal);

    float vdotr = max(dot(viewDir, reflectDir), 0.0);
    vec3 specular = u_Specular * u_LightCol * vec3(pow(vdotr, max((1.0 - u_Roughness) * 256.0, 1.0)));
