...
namespace Shroon
{
    namespace Prism
    {
        namespace HL
        {
            void ES30Buffer::Create(Buffer & buf, BufferSpecification & spec)
            {
            ...
            void ES30Buffer::Destroy(Buffer & buf)
            {
                glDeleteBuffers(1, buf.m_Handle.AsPtrOf<GLuint>());
            }

            void ES30Buffer::Write(Buffer & buf, void * data, size_t size, size_t offset)
            {
                glBindBuffer(buf.m_Type, buf.m_Handle.As<GLuint>());
