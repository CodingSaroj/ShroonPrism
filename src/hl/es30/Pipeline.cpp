...
namespace Shroon
{
    namespace Prism
    {
        namespace HL
        {
            void ES30Pipeline::Create(Pipeline & pl, PipelineSpecification & spec)
            {
                pl.m_Spec = spec;
                ...
                for (uint8_t i = 0; i < 3; i++)
                {
                    if (shaderCompileData[i].ShaderBinary)
                    {
                        ...
                        if (!shaderStatuses[i])
                        {
                            ...
                            ErrorReporter(SHRN_PRISM_LEVEL_FATAL, "OpenGLES3.0::ShaderCompiler", "Failed to compile " + shaderCompileData[i].ShaderType + " shader. Log:");
                            ErrorReporter(SHRN_PRISM_LEVEL_FATAL, "OpenGLES3.0::ShaderCompiler", std::string(log));
                ...
                if (!status)
                {
                    ...
                    ErrorReporter(SHRN_PRISM_LEVEL_FATAL, "OpenGLES3.0::ShaderLinker", "Failed to link shader. Log:");
                    ErrorReporter(SHRN_PRISM_LEVEL_FATAL, "OpenGLES3.0::ShaderLinker", std::string(log));
                }
            ...
            void ES30Pipeline::Destroy(Pipeline & pl)
            {
                glDeleteProgram(pl.m_Handle.As<GLuint>());
            }

            void ES30Pipeline::ApplyStateChanges(Pipeline & pl)
            {
                GLenum enableMask = 0;
