        ...
        namespace HL
        {
            void GL45Pipeline::Create(Pipeline & pl, PipelineSpecification & spec)
            {
                ...
                for (uint8_t i = 0; i < 3; i++)
                {
                    if (shaderCompileData[i].ShaderBinary)
                    {
                        ...
                        if (!shaderStatuses[i])
                        {
                            ...
                            ErrorReporter(SHRN_PRISM_LEVEL_ERROR, "OpenGL4.5::ShaderCompiler", "Failed to compile " + shaderCompileData[i].ShaderType + " shader. Log:");
                            ErrorReporter(SHRN_PRISM_LEVEL_ERROR, "OpenGL4.5::ShaderCompiler", std::string(log));
                ...
                if (!status)
                {
                    ...
                    ErrorReporter(SHRN_PRISM_LEVEL_ERROR, "OpenGL4.5::ShaderLinker", "Failed to link shader. Log:");
                    ErrorReporter(SHRN_PRISM_LEVEL_ERROR, "OpenGL4.5::ShaderLinker", std::string(log));
                }
