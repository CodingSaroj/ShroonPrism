...
namespace Shroon
{
    namespace Prism
    {
        void GL33Framebuffer::Create(Framebuffer & fb, FramebufferSpecification spec)
        {
            ...
            {
                ErrorReporter(SHRN_PRISM_LEVEL_ERROR, "OpenGL3.3::Framebuffer", "Framebuffer incomplete!");
            }
        ...
        void GL33Framebuffer::Enable(Framebuffer & fb)
        {
            glBindFramebuffer(GL_FRAMEBUFFER, fb.m_Handle);
            glViewport(0, 0, fb.m_Spec.Width, fb.m_Spec.Height);
        }

        void GL33Framebuffer::Disable(Framebuffer & fb)
        {
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
        }

        void GL33Framebuffer::SetClearColor(Framebuffer & fb, float r, float g, float b, float a)
        {
            glClearColor(r, g, b, a);
        }

        void GL33Framebuffer::SetClearDepth(Framebuffer & fb, float depth)
        {
            glClearDepth(depth);
        }

        void GL33Framebuffer::SetClearStencil(Framebuffer & fb, uint32_t stencil)
        {
            glClearStencil(stencil);
        }

        void GL33Framebuffer::ClearBuffers(Framebuffer & fb)
        {
