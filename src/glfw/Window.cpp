    ...
    namespace Prism
    {
        ...
        void GLFWWindow::Create(Window & window, WindowSpecification spec)
        {
            switch (s_GraphicsAPI)
            {
                ...
                case API::VK12: APIString = "Vulkan1.2"; break;
            }
            ...
            else if (s_GraphicsAPI == API::VK12)
            {
            ...
            if (s_GraphicsAPI == API::VK12)
            {
                VK12State::Initialize();
            }
        }
        ...
        void GLFWWindow::Update(Window & window, Framebuffer & src, void( * updateFn)(void *), void * userData)
        {
            while (!glfwWindowShouldClose(window.m_Handle))
            {
                ...
                //auto d = e - s;
                //ErrorReporter(SHRN_PRISM_LEVEL_INFO, "PrismTest", "Frametime: " + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(d).count()) + "ms.");
            }

            if (s_GraphicsAPI == API::VK12)
            {
                vkDeviceWaitIdle(VK12State::s_Device);
            }
