...
namespace Shroon
{
    namespace Prism
    {
        void GL45Shader::Create(Shader & shader, ShaderSpecification spec)
        {
            GLuint shaderHandles[6];
            GLint shaderStatuses[6] = {1, 1, 1, 1, 1, 1};
            bool shaderAvailabilities[6];

            memset(shaderAvailabilities, 0, 6);
            ...
            CompileData shaderCompileData[] =
            {
                ...
                {"Tessellation Control", GL_TESS_CONTROL_SHADER, spec.TessellationControlShader},
                {"Tessellation Evaluation", GL_TESS_EVALUATION_SHADER, spec.TessellationEvaluationShader},
                {"Geometry", GL_GEOMETRY_SHADER, spec.GeometryShader},
                {"Fragment", GL_FRAGMENT_SHADER, spec.FragmentShader},
                {"Compute", GL_COMPUTE_SHADER, spec.ComputeShader}
            };
        ...
        void GL45Shader::Destroy(Shader & shader)
        {
            glDeleteProgram(shader.m_Handle);
        }

        void GL45Shader::BindUniformBlock(Shader & shader, std::string name, uint32_t bindingPoint)
        {
